...
module.exports.getAllProducts = async (request, response, next) => {
  try {
    ...
    response
      .status(200)
      .json({ message: "sucessfully get products", products });
  } catch (error) {
    next(error);
  }
};
...
module.exports.getProductById = async (request, response, next) => {
  try {
    ...
    if (productById)
      response
        .status(200)
        .json({ message: "sucessfully get product by id", productById });
...
module.exports.addNewProduct = async (request, response, next) => {
  ...
  try {
    ...
    response
      .status(201)
      .json({ message: "sucessfully add new product", newProduct });
  } catch (error) {
    next(error);
  }
};
...
module.exports.updateProduct = async (request, response, next) => {
  try {
    ...
    if (updatedProduct)
      response
        .status(200)
        .json({ message: "sucessfully update product", updatedProduct });
    else throw new Error("Product doesn't exist");
  } catch (error) {
    next(error);
  }
...
module.exports.deleteProduct = async (request, response, next) => {
  try {
    ...
    if (deletedProduct)
      response
        .status(200)
        .json({ message: "sucessfully delete product", deletedProduct });
    else throw new Error("Product doesn't exist");
  } catch (error) {
    next(error);
  }
